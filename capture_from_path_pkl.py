...
save_dir = 'data\\captured_images_trajectories'
...
for traj_idx, traj_file in enumerate(traj_files):
    ...
    # 保存 traj_data.pkl 文件
    traj_data = {
        "position": np.array(positions),      # shape: (T, 3), [x, y, z]
        "orientation": np.array(orientations)  # shape: (T, 4), [w, x, y, z]
    }
    
    pkl_file = os.path.join(traj_save_path, 'traj_data.pkl')
    with open(pkl_file, 'wb') as f:
        pickle.dump(traj_data, f)
    
    print(f"Saved {num_points} images and traj_data.pkl to {traj_save_path}")
